...
def create_representation_dataset(window_size, train_x, train_y, test_x, test_y, batch_size, input_representation = 0):
    x = train_x
    y = train_y
    x_test = test_x
    y_test = test_y
    ...
    # 받아오는 representation을 설정하고 window_size 구간 내 데이터를 representation으로 대체
    # train/validation/test 데이터를 window_size 시점 길이로 분할
    datasets = []
    for set in [(x_train, y_train, n_train), (x_valid, y_valid, n_valid), (x_test, y_test, n_test)]:
        T = set[0].shape[-1]
        ...
        # 나중에는 length 길이에 맞게 전체 feature.shape을 받으면 될듯
        # 나중에는 input_representation을 직접 사용 (representation = input_representation)
